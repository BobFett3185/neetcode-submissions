...
class Solution:
    def mergeTwoLists(self, list1: Optional[ListNode], list2: Optional[ListNode]) -> Optional[ListNode]:
        ...
        node.next = current1 or current2 # this fills in the rest of the nodes
        return dummy.next # return the 2nd element which is head

# when dealing with linked list -- mess with the .next ptr 

# build the chain -- node.next = some node 
# then move the hand -- node = node.next
